## Purpose
This is my first ever project with a GUI.<br>
This project is a scientific calculator that can compute basic mathmatical, trigonometric, logarthmic, and statistical expressions.

## Installation
To run this project, it requires that you have java 11 or higher and javafx 17 or higher installed onto your system.

## Usage
To run this project, include the bin and lib folders from your javafx folder in the directory this program is in.  
### Run the App:
```
javac --module-path lib --add-modules javafx.controls *.java
java --module-path lib --add-modules javafx.controls CalculatorDriver
```

## Images
### Starting Screen:
![The starting screen of the calculator](images/starting_screen.png)
<br>
### Calculations:
![The calculator performing a calculation](images/calculation.png)
<br>
### Making a Data set:
![The Calculator creating a dataset](images/stats.png)
